...
export const Coin = (value, script) =>
({
  value: value,
  script: script,
...
const ft = (targets, feePerByte, coins) => {
  ...
  if (isEmpty(selection)) {
    ...
    if (maxBalance < target + fee) {
      ...
      if (extra >= dustThreshold(feePerByte)) {
        // add change
        return { fee: fee, inputs: selectedCoins, outputs: [...targets, Coin(extra)] }
      } else {
        // burn change
...
// singleRandomDraw :: Number -> [Coin(a), ..., Coin(b)] -> Selection
export const selectAll = (feePerByte, coins) => {
  const effectiveCoins = filter(c => effectiveValue(feePerByte, c) > 0, coins)
  ...
  return {
    ...
    outputs: [Coin(effBalance)]
  }
}
// singleRandomDraw :: [Coin(x), ..., Coin(y)] -> Number -> [Coin(a), ..., Coin(b)] -> String -> Selection
export const singleRandomDraw = (targets, feePerByte, coins, seed) => {
  const rng = is(String, seed) ? seedrandom(seed) : undefined
  return findTarget(targets, feePerByte, shuffle(coins, rng))
}
...
const bnb = (targets, feePerByte, coins, seed) => {
  ...
  if (isEmpty(bnbSelection)) {
    return singleRandomDraw(targets, feePerByte, sortedCoins, seed)
  } else {
    return {

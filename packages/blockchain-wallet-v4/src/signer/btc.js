...
export const signWithLockbox = function* (selection, transport, scrambleKey, changeIndex, api) {
  const BTC = new Btc(transport, scrambleKey)
  const inputs = []
  const paths = []
  const changePath = `44'/0'/0'/M/1/${changeIndex}`
  for (const i in selection.inputs) {
  ...
  const txHex = yield BTC.createPaymentTransactionNew(inputs, paths, changePath, outputs)
  const txId = crypto

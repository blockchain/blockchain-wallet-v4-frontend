...
export const parseBIP38toECPair = function (base58Encrypted, passphrase, network) {
  ...
  var verifyHashAndReturn = function () {
    var tmpkey = new Bitcoin.ECPair(decrypted, null, {compressed: isCompPoint, network: network})
  ...
  if (!isECMult) {
    ...
    var decryptedBytes = WalletCrypto.AES.decrypt(Buffer.from(hex.slice(7, 7 + 32), 'hex'), k, null, AESopts)
    ...
    var ownersalt = Buffer.from(!hasLotSeq ? ownerentropy : ownerentropy.slice(0, 4), 'hex')
    ...
    if (!hasLotSeq) {
      ...
      var prefactorB = Buffer.concat([prefactorA, Buffer.from(ownerentropy, 'hex')])
      passfactor = hash256(prefactorB)
    }

    var kp = new Bitcoin.ECPair(BigInteger.fromBuffer(passfactor), null, {network: network})
    ...
    var encryptedpart1 = Buffer.concat([Buffer.from(hex.slice(15, 15 + 8), 'hex'), Buffer.from(unencryptedpart2Bytes.slice(0, 0 + 8), 'hex')])
    ...
    var seedb = Buffer.concat([Buffer.from(unencryptedpart1Bytes.slice(0, 0 + 16), 'hex'), Buffer.from(unencryptedpart2Bytes.slice(8, 8 + 8), 'hex')])

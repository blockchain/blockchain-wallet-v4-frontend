...
export default ({ api }) => {
  ...
  const fetchTransactions = function * ({ type, payload }) {
    ...
    try {
      ...
      const btcHDAccountContext = yield select(selectors.core.wallet.getHDAccounts)
      const context = yield select(selectors.core.kvStore.bch.getContext, btcHDAccountContext)

...
export const getAllCoinRatesSelector = (state): any => {
  return {
    AAVE: selectors.data.eth.getErc20Rates(state, 'aave').getOrElse(missingRatesFallback),
    ...
    PAX: selectors.data.eth.getErc20Rates(state, 'pax').getOrElse(missingRatesFallback),
    USDT: selectors.data.eth.getErc20Rates(state, 'usdt').getOrElse(missingRatesFallback),
    WDGLD: selectors.data.eth.getErc20Rates(state, 'wdgld').getOrElse(missingRatesFallback),
    XLM: selectors.data.xlm.getRates(state).getOrElse(missingRatesFallback),
    YFI: selectors.data.eth.getErc20Rates(state, 'yfi').getOrElse(missingRatesFallback)
  }
}

...
export const getDefaultAccountId = (state) => getDefaultAccount(state).map(prop('publicKey'))
...
export const getXlmTxNotes = (state) => getMetadata(state).map(path(['value', 'tx_notes']))

export const getXlmTxNote = (state, txHash) => getXlmTxNotes(state).map(prop(txHash))

export const getContext = (state) => getAccounts(state).map(map(prop('publicKey')))

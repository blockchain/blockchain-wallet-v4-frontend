...
export const getDevice = (state, deviceIndex) => getDevices(state).map(nth(deviceIndex))
...
export const getLockboxBtcContext = (state) => {
  return getLockboxBtcAccounts(state).map((accounts) => {
    return accounts
      ? accounts.map((a) =>
          path(['derivations'], a).filter(propEq('type', 'legacy')).map(prop('xpub'))
        )
      : []
  })
}
export const getBtcContextForDevice = (state, deviceIndex) =>
  getDevice(state, deviceIndex)
    ...
    .map(map((a) => path(['derivations'], a).filter(propEq('type', 'legacy')).map(prop('xpub'))))

export const getLockboxBtcAccount = (state, xpub) =>
  getLockboxBtcAccounts(state)
    .map(filter((x) => propEq('xpub', xpub, x.derivations.find(propEq('type', 'legacy')))))
    .map(head)

export const getDeviceFromBtcXpubs = (state, xpubs) => {
  const accountContainsXpubs = (account) =>
    any((xpub) => propEq('xpub', xpub, account.derivations.find(propEq('type', 'legacy'))), xpubs)
  const deviceFilter = (device) => any(accountContainsXpubs, path(['btc', 'accounts'], device))
  return getDevices(state).map(filter(deviceFilter)).map(head)
}
...
export const getLockboxBchContext = (state) => {
  return getLockboxBchAccounts(state).map((accounts) => {
    return accounts
      ? accounts.map((a) =>
          path(['derivations'], a).filter(propEq('type', 'bch-145')).map(prop('xpub'))
        )
      : []
  })
}
...
export const getBchContextForDevice = (state, deviceIndex) =>
  getDevice(state, deviceIndex)
    ...
    .map(map((a) => path(['derivations'], a).filter(propEq('type', 'bch-145')).map(prop('xpub'))))

export const getLockboxBchAccount = (state, xpub) =>
  getLockboxBchAccounts(state)
    .map(filter((x) => propEq('xpub', xpub, x.derivations.find(propEq('type', 'bch-145')))))
    .map(head)

export const getDeviceFromBchXpubs = (state, xpubs) => {
  const accountContainsXpubs = (account) =>
    any((xpub) => propEq('xpub', xpub, account.derivations.find(propEq('type', 'bch-145'))), xpubs)
  const deviceFilter = (device) => any(accountContainsXpubs, path(['bch', 'accounts'], device))
  return getDevices(state).map(filter(deviceFilter)).map(head)
}
...
export const getDeviceFromEthAddr = (state, addr) => {
  ...
  const deviceFilter = (device) => any(accountContainsAddr, path(['eth', 'accounts'], device))
  return getDevices(state).map(filter(deviceFilter)).map(head)
}
...
export const getDeviceFromXlmAddr = (state, publicKey) => {
  ...
  const deviceFilter = (device) => any(accountContainsAddr, path(['xlm', 'accounts'], device))
  return getDevices(state).map(filter(deviceFilter)).map(head)
}

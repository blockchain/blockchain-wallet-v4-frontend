...
export const getContext = compose(flatten, intoJS, Wallet.selectContext, getWallet)
...
export const getAccountXpub = curry((index, state) =>
  compose(HDAccount.selectXpub, HDWallet.selectAccount(index), getDefaultHDWallet)(state)
)
export const getAccountLabel = curry((state, index) =>
  compose(HDAccount.selectLabel, HDWallet.selectAccount(index), getDefaultHDWallet)(state)
)
...
export const getSpendableActiveAddresses = compose(Wallet.spendableActiveAddresses, getWallet)
export const getAddress = curry((address, state) =>
  compose(Wallet.getAddress(address), getWallet)(state)
)
export const isWrapperLatestVersion = compose(Wrapper.isLatestVersion, getWrapper)

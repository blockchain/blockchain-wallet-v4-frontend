...
export const getCoin = path(['adapter', 'components', 'price_chart', 'coin'])

export const getTime = path(['adapter', 'components', 'price_chart', 'time'])

export const getData = state => {
  const currency = 'USD'
  const coin = getCoin(state)
  const time = getTime(state)
  const priceIndexSeriesDataR = selectors.data.misc.getPriceIndexSeries(state)

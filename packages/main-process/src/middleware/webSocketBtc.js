  ...
  return next => action => {
    ...
    if (type === actionTypes.middleware.webSocket.btc.START_SOCKET) {
      socket.connect(
        compose(
          store.dispatch,
          actions.middleware.webSocket.btc.openSocket
        ),
        compose(
          store.dispatch,
          actions.middleware.webSocket.btc.messageSocket
        ),
        compose(
          store.dispatch,
          actions.middleware.webSocket.btc.closeSocket
        )
      )
    }

    if (type === actionTypes.middleware.webSocket.btc.STOP_SOCKET) {
      socket.close()
    }

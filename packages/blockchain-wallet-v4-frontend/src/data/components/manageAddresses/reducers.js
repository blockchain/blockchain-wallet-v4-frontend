...
export default (state = {}, action) => {
  ...
  switch (type) {
    case AT.TOGGLE_USED_ADDRESSES: {
      ...
      return assocPath([walletIndex, derivation, 'usedAddressesVisible'], payload.visible, state)
    }
    ...
    case AT.FETCH_UNUSED_ADDRESSES_LOADING: {
      const { derivation, walletIndex } = payload
      return assocPath([walletIndex, derivation, 'unusedAddresses'], Remote.Loading, state)
    }
    ...
    case AT.FETCH_USED_ADDRESSES_ERROR: {
      ...
      return assocPath([walletIndex, derivation, 'usedAddresses'], Remote.Failure(message), state)
    }
    case AT.FETCH_USED_ADDRESSES_LOADING: {
      const { derivation, walletIndex } = payload
      return assocPath([walletIndex, derivation, 'usedAddresses'], Remote.Loading, state)
    }

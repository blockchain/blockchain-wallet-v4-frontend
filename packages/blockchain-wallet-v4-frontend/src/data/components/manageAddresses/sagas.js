...
export default ({ api, networks }) => {
  ...
  const deriveAddresses = function (account, receiveIndex, derivation) {
    ...
    while (i <= receiveIndex) {
      addrs.push(Types.HDAccount.getReceiveAddress(account, i, networks.btc, derivation))
  ...
  const fetchUnusedAddresses = function* (action) {
    ...
    try {
      ...
      const labels = Types.HDAccount.selectAddressLabels(account, derivation).reverse().toArray()
      // derive addresses from label indexes
      const labeledAddrs = labels.map((la) => ({
        address: Types.HDAccount.getReceiveAddress(account, la.index, networks.btc, derivation),
        index: la.index,
        label: la.label
      }))
      ...
      forEach((labeledAddr) => {
        const idx = findIndex(propEq('address', labeledAddr.address))(unusedAddresses)
        if (idx !== -1) {
  ...
  const fetchUsedAddresses = function* (action) {
    ...
    try {
      ...
      const wallet = yield select(selectors.core.wallet.getWallet)
      const account = Types.Wallet.selectHDAccounts(wallet).get(walletIndex)
      const xpub = Types.HDAccount.selectXpub(account, derivation)
      // get current receive index of wallet
      const receiveIndex = yield select(selectors.core.data.btc.getReceiveIndex(xpub))
      ...
      forEach((labeledAddr) => {
        const idx = findIndex(propEq('address', labeledAddr.address))(usedAddresses)
        if (idx !== -1) {
          usedAddresses[idx].label = labeledAddr.label
        }
      }, labeledAddrs)

      yield put(A.fetchUsedAddressesSuccess(walletIndex, derivation, usedAddresses))
    } catch (e) {
  ...
  const editAddressLabel = function* (action) {
    const { accountIndex, addressIndex, derivation, walletIndex } = action.payload
    try {
      ...
      yield put(
        actions.core.wallet.setHdAddressLabel(accountIndex, addressIndex, derivation, newLabel)
      )
      yield put(A.fetchUnusedAddresses(walletIndex, derivation))
  ...
  const editImportedAddressLabel = function* (action) {
    ...
    try {
      ...
      yield put(actions.alerts.displaySuccess(C.UPDATE_IMPORTED_ADDRESS_LABEL_SUCCESS))
    } catch (e) {
      yield put(A.editImportedAddressLabelError(address, e))
      yield put(actions.logs.logErrorMessage(logLocation, 'editImportedAddressLabel', e))
      yield put(actions.alerts.displayError(C.UPDATE_IMPORTED_ADDRESS_LABEL_ERROR))
    }
  }
  ...
  const deleteAddressLabel = function* (action) {
    ...
    try {
      ...
      yield call(
        function* () {
          yield put(actions.core.wallet.deleteHdAddressLabel(accountIdx, addressIdx, derivation))
        },
        accountIdx,
        addressIdx
      )

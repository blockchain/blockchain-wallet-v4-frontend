...
import { PERSONAL_STEPS, EMAIL_STEPS, SMS_STEPS, EMAIL_FORM, SMS_NUMBER_FORM } from './model'

export const logLocation = 'components/identityVerification/sagas'

export default ({ api, coreSagas }) => {
  const derivePersonalStep = ({ email, emailVerified, smsNumber, smsVerified }) => {
  ...
  const updatePersonalStep = function * (action) {
    ...
    yield put(A.setPersonalStep(derivePersonalStep({ email, emailVerified, smsNumber, smsVerified })))
  ...
  const resendSmsCode = function * () {
    const smsNumber = (yield select(selectors.core.settings.getSmsNumber)).getOrElse('')
    yield put(actions.modules.settings.updateMobile(smsNumber))
  }

...
export default () => {
  const initialized = function* (action: ReturnType<typeof A.initialized>) {
    try {
      ...
      yield put(actions.core.data.fiat.fetchTransactions(action.payload.currency, true))
    } catch (e) {
  ...
  const loadMore = function* (action: ReturnType<typeof A.loadMore>) {
    try {
      yield put(actions.core.data.fiat.fetchTransactions(action.payload.currency))
    } catch (e) {

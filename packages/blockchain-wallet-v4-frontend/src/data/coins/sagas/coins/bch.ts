...
export const getDefaultAccount = function * () {
  ...
  const bchDefaultIndex = (yield select(selectors.core.kvStore.bch.getDefaultAccountIndex)).getOrElse(0)
...
export const getNextReceiveAddress = function * (coin, networks) {
  ...
  const defaultAccountIndex = (yield select(selectors.core.kvStore.bch.getDefaultAccountIndex)).getOrFail()
  const nextAddress = selectors.core.common.bch.getNextAvailableReceiveAddress(networks.bch, defaultAccountIndex, state).getOrFail('Failed to get BCH receive address')

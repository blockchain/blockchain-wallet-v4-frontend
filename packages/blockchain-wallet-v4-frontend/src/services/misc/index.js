...
const checkForVulnerableAddressError = (message) => {
  ...
  const re =
    /A security issue effects address [13][a-km-zA-HJ-NP-Z0-9]{25,34}. Please Archive It and Contact support@blockchain.zendesk.com/
  const testForErrorMatch = match(re, message)
  const addressMatch =
    !isEmpty(testForErrorMatch) && match(/[13][a-km-zA-HJ-NP-Z0-9]{26,33}/, message)

...
export const statusHelper = status => {
  switch (status) {
    ...
    case 'expired': return { color: 'error', text: <FormattedMessage id='scenes.buysellorderhistory.list.orderstatus.expired' defaultMessage='Expired' /> }
...
export const bodyStatusHelper = (status, isBuy) => {
  if (isBuy) {
    ...
    switch (status) {
      case 'awaiting_transfer_in':
      case 'processing': return { text: <FormattedMessage id='scenes.buysellorderhistory.list.orderstatusbody.sell.coinify.processing' defaultMessage='Coinify will issue the owed sale amount directly to your bank account within 2 business days. You will receive an email from Coinify to confirm the transfer, as soon as they issue the funds to your account.' /> }
      ...
      case 'expired': return { text: <FormattedMessage id='scenes.buysellorderhistory.list.orderstatusbody.sell.expired' defaultMessage='Your sell trade expired.' /> }
    }
  }

...
export const getData = (state: RootState, { side }: OwnProps) => {
  ...
  const pairs = selectors.components.swap.getPairs(state).getOrElse(null)

  if (!pairs) return { accounts }
  ...
  const _checkCoinSelected = (account: SwapAccountType) => {
    if (side === 'COUNTER' && values?.BASE?.coin === account.coin) return true
    if (side === 'BASE' && values?.COUNTER?.coin === account.coin) return true
    return false
  }

  const _checkBaseAccountZero = (account: SwapAccountType) => {
    if (account.balance.toString() === '0' && side === 'BASE') return true
  ...
  const accountsForSide = coinsForSide.reduce((prevValue, curValue: CoinType) => {
    if (!prevValue[curValue]) {
      prevValue[curValue] = accounts[curValue]
    }

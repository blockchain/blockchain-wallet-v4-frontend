...
export const getData = (state) => ({
  personalData: selectors.components.identityVerification.getPersonalData(state),
  step: selectors.components.identityVerification.getPersonalStep(state).getOrElse(null),
  countryCode: getCountryCode(state)
})

export const getEmailData = (state) => ({
  step: selectors.components.identityVerification.getEmailStep(state).getOrElse(null),
  emailVerifiedError: path(['securityCenter', 'emailVerifiedError'], state)
})

export const getSmsData = (state) => ({
  step: selectors.components.identityVerification.getSmsStep(state).getOrElse(null),

...
import { tradeDetails, statusHelper, kycBodyStatusHelper } from 'services/CoinifyService'
...
const Kyc = ({ trade, close }) => {
  const headerStatus = statusHelper(trade.state)
  const kycBodyStatus = kycBodyStatusHelper(trade.state, trade.isBuy)
    ...
    <Fragment>
      <ModalHeader onClose={close}>
        <Text color={headerStatus.color}>
          <FormattedMessage id='order_details.identity_verification' defaultMessage='Identity Verification' />
        </Text>
      </ModalHeader>
      <ModalBody>

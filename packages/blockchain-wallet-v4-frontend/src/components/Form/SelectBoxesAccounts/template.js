...
const SelectBoxesAccounts = (props) => {
  const { source, sourceElements, target, targetElements, handleSelectSource, handleSelectTarget, handleSwap, ...rest } = props

  return (
    <Wrapper>
      <SelectInput elements={sourceElements} value={source} onChange={handleSelectSource} {...rest} />
      <SwapIcon name='exchange-2' size='24px' weight={500} cursor onClick={handleSwap} />
      <SelectInput elements={targetElements} value={target} onChange={handleSelectTarget} {...rest} />
    </Wrapper>
  )
}

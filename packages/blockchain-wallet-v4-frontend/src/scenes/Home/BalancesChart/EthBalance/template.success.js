...
const EthBalance = (props) => {
  const { btcBalance, bchBalance, ethBalance, handleCoinDisplay, partner, ...rest } = props
  ...
  return (
    <Fragment>
      ...
      <CoinBalance onClick={handleCoinDisplay}>
        {
          !gte(ethBalance, 0)
            ...
            : <SwitchableDisplay coin='ETH' cursor='pointer' size='14px' weight={200}>
              {ethBalance}
            </SwitchableDisplay>
        }
      </CoinBalance>
      {(gt(btcBalance, 0) || gt(bchBalance, 0)) && !gt(ethBalance, 0)
        ...
        : gt(0, ethBalance) &&
          <Link size='10px' weight={300} onClick={() => modalsActions.showModal('RequestEther')}>

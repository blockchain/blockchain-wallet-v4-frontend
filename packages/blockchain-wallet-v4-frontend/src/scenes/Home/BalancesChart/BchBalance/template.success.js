...
const BchBalance = (props) => {
  const { btcBalance, bchBalance, ethBalance, bchAccountsLength, handleCoinDisplay, partner, ...rest } = props
  ...
  return (
    <Fragment>
          ...
          !gte(bchBalance, 0)
            ? <Fragment>
              ...
              <Link size='12px' weight={200} onClick={handleRefresh}>
                <FormattedMessage id='scenes.home.balanceschart.bch.refresh' defaultMessage='Refresh' />
              </Link>
            </Fragment>
            : <SwitchableDisplay coin='BCH' cursor='pointer' size='14px' weight={200}>
              {bchBalance}
            </SwitchableDisplay>
        }
      </CoinBalance>
      {(gt(btcBalance, 0) || gt(ethBalance, 0)) && !gt(bchBalance, 0)
        ...
        : gt(0, bchBalance) &&
          <Link size='10px' weight={300} onClick={() => modalsActions.showModal('RequestBch')}>

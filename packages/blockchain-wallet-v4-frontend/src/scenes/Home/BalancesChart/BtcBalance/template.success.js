...
import { CoinBalance, ColourBar, ViewAllText, WalletLink } from 'components/BalancesChart'
import SwitchableDisplay from 'components/Display/SwitchableDisplay'

const BtcBalance = (props) => {
  const { btcBalance, bchBalance, ethBalance, btcAccountsLength, handleCoinDisplay, partner, ...rest } = props
    ...
    <Fragment>
          ...
          !gte(btcBalance, 0)
            ? <Fragment>
              ...
              <Link size='12px' weight={200} onClick={handleRefresh}>
                <FormattedMessage id='scenes.home.balanceschart.btc.refresh' defaultMessage='Refresh' />
              </Link>
            </Fragment>
            : <SwitchableDisplay coin='BTC' cursor='pointer' size='14px' weight={200}>
              {btcBalance}
            </SwitchableDisplay>
        }
      </CoinBalance>
      {partner
        ...
        : gt(ethBalance, 0) || gt(bchBalance, 0)
          ...
          : gt(0, btcBalance) &&
            <Link size='10px' weight={300} onClick={() => modalsActions.showModal('RequestBitcoin')}>

  ...
  return (
    <Wrapper success={success}>
      <EmailCodeWrapper onSubmit={handleSubmitVerification}>
        <Field
          name='emailCode'
          validate={[validEmailCode, required]}
          component={TextBox}
          placeholder='123AB'
        />
        <Button nature='primary' type='submit' disabled={!emailCode}>
          <FormattedMessage
            id='scenes.preferences.email.settings.updateform.verify'
      ...
      {failed ? (
        <ErrorMessage inline>
          <Text size='12px' weight={300} color='red'>
            <FormattedMessage
              id='scenes.security.email.verificationwrong'
              defaultMessage='Your verification code is incorrect. Please double check your email and try again.'
            />
          </Text>
          <span>&nbsp;</span>
          <Link size='12px' onClick={handleResend}>

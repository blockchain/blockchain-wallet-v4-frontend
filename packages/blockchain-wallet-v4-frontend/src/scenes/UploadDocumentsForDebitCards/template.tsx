...
const renderAdditionalSectionIDAndPassport = () => {
  return (
    <AdditionalInfoWrapper>
      <Text color='blue900' size='16px' weight={500} style={{ paddingBottom: 0 }}>
        <FormattedMessage
          id='scenes.uploaddoc.title'
          defaultMessage='Before uploading, be sure that your document:'
        />
      </Text>
      <ul>
        <li>
...
const UploadDocuments = ({
  ...
  const isSelfiePresent =
    data && data.documentsTypes.filter((doc) => doc.toLowerCase().includes('selfie')).length > 0
      ...
      <TextContainer>
        ...
        <Text color='blue900' size='16px' weight={400}>
          <FormattedMessage
            ...
            defaultMessage='We need additional information to verify your identity in order to process your application for the Blockchain.com Visa Card.'
          />
        </Text>
        <Text color='blue900' size='16px' weight={400}>
          <FormattedMessage
...
type Props = {
  data: any
  deleteFileAt: Function
  dropzoneRef: RefObject<any>
  files: Array<any>
  loading: boolean

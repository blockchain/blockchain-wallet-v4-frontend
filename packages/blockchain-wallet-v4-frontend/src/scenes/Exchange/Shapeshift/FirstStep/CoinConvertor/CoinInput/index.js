...
class CoinInputContainer extends React.Component {
  constructor (props) {
    ...
    const fiat = convertCoinToFiat(coin, coinName, coinName, currency, bchRates, bchRates, btcRates, ethRates)
  ...
  componentWillReceiveProps (nextProps) {
    ...
    const { coinName, coin, currency, bchRates, btcRates, ethRates } = nextProps
    ...
    if (!equals(this.props.coin, coin) || !equals(this.props.coinName, coinName)) {
      const fiat = convertCoinToFiat(coin, coinName, coinName, currency, bchRates, btcRates, ethRates)
  ...
  handleChangeCoin (e) {
    ...
    const { coinName, currency, bchRates, btcRates, ethRates } = this.props
    const fiat = convertCoinToFiat(coin, coinName, coinName, currency, bchRates, btcRates, ethRates)
    this.setState({ coin, fiat })
  ...
  handleChangeFiat (e) {
    ...
    const { coinName, currency, bchRates, btcRates, ethRates } = this.props
    const coin = convertFiatToCoin(fiat, coinName, coinName, currency, bchRates, btcRates, ethRates)
    this.setState({ coin, fiat })
    this.props.handleChange(coin)
  }

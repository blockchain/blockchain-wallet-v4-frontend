...
Poly1305.prototype.init = function init(key) {
  ...
  console.info('init poly with key '+key.toString('hex'))
...
Poly1305.prototype.blocks = function blocks(data, bytes, m) {
  ...
  while (bytes >= 16) {
    console.info('process blocks '+this.h)
    // h += m[i]
...
Poly1305.prototype.update = function update(data) {
  console.info('poly update data: '+data.toString('hex'))
...
Poly1305.prototype.finish = function finish() {
  console.log('finish - current state is '+this.h)
  ...
  console.log('finish - write h to mac: '+this.h)


  for (let i = 0; i < 8; i++)
    mac.writeUInt16LE(this.h[i], i * 2, true);

  console.log('mac is '+mac.toString('hex'))

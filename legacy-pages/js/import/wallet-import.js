    ...
    modal
      ...
      .click(function () {
        ...
        if (
          ...
          tpassword.length == 0 ||
          tpassword.length < 10 ||
          tpassword.length > 255
        ) {
          MyWallet.makeNotice(
            'error',
            'misc-error',
            'Password must be 10 characters or more in length'
          )
          return
        }
  ...
  function generateUUIDs(n, success, error) {
    $.ajax({
      ...
      success: function (data) {
        if (data.uuids && data.uuids.length == n) success(data.uuids)
        ...
        function (uuids) {
          try {
            ...
            reallyInsertWallet(guid, sharedKey, password, function () {
              ...
              window.confirm(
                `Your wallet has been recovered successfully! You will now be redirected to the login page with your new wallet ID prefilled.  Use the password you created to login into your wallet.`
              )
